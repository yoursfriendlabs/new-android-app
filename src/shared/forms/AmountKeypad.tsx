...
export function AmountKeypad({ onChange, value }: AmountKeypadProps) {
  ...
  function handleKeyPress(key: string) {
    ...
    const nextExpression =
      key === '.' && /(^|[+\-×÷−])$/.test(expression) ? `${expression}0.` : `${expression}${key}`;
  ...
  return (
    <View style={styles.container}>
      <View style={[styles.displayCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
        <Text style={[styles.caption, { color: colors.textSoft }]}>{formatCurrency(value || 0)}</Text>
            ...
            {row.map((key) => (
              <Pressable
                ...
                style={({ pressed }) => [
                  ...
                  key === '=' && { backgroundColor: colors.primary, borderColor: colors.primary },

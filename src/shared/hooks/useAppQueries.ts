...
export function invalidatePartyQueries(queryClient: QueryClient, partyIds: string[] = []) {
  ...
  uniquePartyIds.forEach((partyId) => {
    jobs.push(
      ...
      queryClient.invalidateQueries({ queryKey: ['party-detail-report', partyId] }),
      queryClient.invalidateQueries({ queryKey: ['party-statement', partyId] }),
      queryClient.invalidateQueries({ queryKey: ['party-transactions', partyId] }),
    );
  });
...
export function useLedger(partyId?: string, range?: { from?: string; to?: string }) {
  return useQuery<LedgerEntry[]>({
    queryKey: ['ledger', partyId ?? 'all', range?.from ?? 'all', range?.to ?? 'all'],
    queryFn: async () =>
      withFallback(
        async () => {
...
export function usePartyTransactions(partyId?: string, direction?: string) {
  return useQuery<PartyTransaction[]>({
    ...
    queryFn: async () => {
      const response = await partyTransactionsApi.list({ partyId, direction, limit: 200 });
      return extractListItems<PartyTransaction>(response)
        .map((item) => ({
          ...item,
...
export function useTables(query = {}, options?: { enabled?: boolean }) {

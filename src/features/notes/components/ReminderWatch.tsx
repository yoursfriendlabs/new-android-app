...
export function ReminderWatch() {
  ...
  useEffect(() => {
    ...
    const open = (response: any) => {
      ...
      // Pekka's daily moments open Pekka itself, not a saved reminder.
      const pekkaMorning = reminderId.startsWith(MORNING_ID_PREFIX);
      const pekkaClose = reminderId.startsWith(DAY_CLOSE_ID_PREFIX);
      const pekkaCollect = reminderId.startsWith(COLLECT_ID_PREFIX);
      if (reminderId && !pekkaMorning && !pekkaClose && !pekkaCollect) {
        void useHabitStore.getState().markPingFired(reminderId);
      }
      ...
      try {
        ...
        else if (pekkaClose) usePekkaStore.getState().openDayClose();
        else if (pekkaCollect) usePekkaStore.getState().openTips();
      } catch {
        // Navigation fallback
      }
    };

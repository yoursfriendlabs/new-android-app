...
import { useBankAccounts } from '@/src/features/money/hooks/useBankAccounts';
import { useMoneyFeed } from '@/src/features/money/hooks/useMoneyFeed';
...
import type { BankAccount, MoneyFeedItem } from '@/src/types/models';
...
export function BanksScreen() {
  ...
  const accounts = bankQuery.items;
  const totalBalance = bankQuery.totalBalance;
  ...
  const debouncedSearch = useDebouncedValue(txSearch);
  const history = useMoneyFeed({
    bankId: selectedBank?.id,
    includeCash: selectedBank?.name.toLowerCase().includes('cash'),
    amountBasis: 'paid',
    search: debouncedSearch.trim() || undefined,
  }, Boolean(selectedBank));

  async function handleTransactionBill(item: MoneyFeedItem) {
    try {
      await openFeedReceipt(item, businessProfile, selectedBank?.name);
  ...
  async function handleRefreshAll() {
    await Promise.all([bankQuery.refetch(), ...(selectedBank ? [history.refetch()] : [])]);
  }
  ...
  return (
    ...
    >
      <ScrollView
        ...
        refreshControl={<RefreshControl refreshing={bankQuery.isRefreshing} onRefresh={() => void handleRefreshAll()} />}
        {...loadMoreOnScroll(bankQuery.loadMore)}
      ...
      >
        ...
        <View style={[styles.totalCard, { backgroundColor: colors.successSoft, borderColor: colors.border }]}>
          ...
          <Text style={[styles.totalValue, { color: colors.success }]}>{totalBalance === undefined ? '—' : formatCurrency(totalBalance, currency)}</Text>
        </View>

        {bankQuery.isLoading ? <ActivityIndicator color={colors.primary} /> : null}
        {bankQuery.isError ? (
          <Pressable onPress={() => void bankQuery.refetch()}><Text style={{ color: colors.danger }}>Could not refresh accounts. Tap to retry.</Text></Pressable>
        ) : null}
        {!accounts.length && !bankQuery.isLoading && !bankQuery.isError ? (
          <EmptyState
            icon="bank-plus"
        ...
        <ListFooterLoader list={bankQuery} />
      </ScrollView>
      ...
      >
        {selectedBank ? (
          <View style={styles.detailContent}>
            ...
            {history.isError ? (
              <Pressable onPress={() => void history.refetch()}><Text style={{ color: colors.danger }}>Could not refresh transactions. Tap to retry.</Text></Pressable>
            ) : null}
            {/* Transactions List */}
            {history.items.length > 0 ? (
              <View style={styles.txListContainer}>
                {history.items.map((tx) => (
                  <Pressable
                    key={tx.id}
                    onPress={() => void handleTransactionBill(tx)}
                    style={({ pressed }) => [
                      styles.txRowCard,
                      { backgroundColor: colors.surface, borderColor: colors.border },
                    ...
                    <View style={styles.txLeft}>
                      <View
                        style={[
                          ...
                          {
                            backgroundColor:
                              tx.kind === 'in' ? colors.successSoft : colors.dangerSoft,
                          },
                        ]}
                      >
                      ...
                      <View style={styles.txTexts}>
                        ...
                        <Text style={[styles.txMeta, { color: colors.textMuted }]}>
                          {tx.category} • {prettyDate(tx.date)}
                        </Text>
                        {tx.note ? (
